# day3_lamport.py
# Goal: Replace wall-clock time with Lamport logical clocks for ordering.
# Run: python day3_lamport.py
...
def sender(out_q: mp.Queue, n: int = 12, gap_s: float = 0.02) -> None:
    ...
    lc = 0
    for i in range(n):
        lc += 1  # Lamport rule: increment before send/event
...
def network(in_q: mp.Queue, out_q: mp.Queue, delay_min: float = 0.0, delay_max: float = 0.35) -> None:
    """
    Simulated network that can reorder messages via concurrent delivery threads.
    Each message is delayed independently, so arrival order may differ from send order.
    """
    ...
    while True:
        msg = in_q.get()
        if msg is None:
            # Stop intake, flush in-flight deliveries, then close downstream.
            for t in threads:
                t.join()
            out_q.put(None)
            break
...
def receiver(in_q: mp.Queue) -> None:
    ...
    print(
        ...
        "but it doesn't prove causality for all pairs (that’s Day 4)."
    )


def main() -> None:
    mp.set_start_method("spawn", force=True)  # macOS-friendly
